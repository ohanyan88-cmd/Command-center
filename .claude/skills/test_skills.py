...
class T05_Execution(unittest.TestCase):
    ...
    def test_information_retrieval(self):
        r = engine.run_skill(REG, "information_retrieval", {"query": "Daily check"}); self.assertEqual(r["status"], "EXECUTED"); self.assertIn("hits", r["result"])
...
class T07_VerificationAudit(unittest.TestCase):
    ...
    def test_completion_verification(self):
        r = engine.run_skill(REG, "completion_verification", {"evidence_spec": {"type": "file_exists", "path": "Առաջադրանքներ.xlsx"}}); self.assertEqual(r["status"], "VERIFIED")
